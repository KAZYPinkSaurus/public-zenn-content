...
|| イメージサイズ |Docker Pullに要した時間^[ap-northeast-1での実験結果] |
|----| ---- | ---- | 
|Deep Learning Containr^[pytorch-inference-neuron:1.12.1-neuron-py38-sdk2.5.0-ubuntu20.04]|4.1GB|135sec|
| 自作コンテナ| **1.7GB** | **61sec** |
...
Inf1 インスタンスをいざ運用しようと[AWSのドキュメント](https://docs.aws.amazon.com/ja_jp/AmazonECS/latest/developerguide/ecs-inference.html)を読むと、[Deep Learning Container](https://github.com/aws/deep-learning-containers/blob/master/available_images.md#neuron-inference-containers)の案内があるので、そのまま使っている方も多いのではないでしょうか。
...
763104351884.dkr.ecr.ap-northeast-1.amazonaws.comからイメージをプルするのにも、わたしの環境では
...
ベースとなるubuntuのイメージは、[2022年11月にpython3.8に対応](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/release-notes/prev/rn.html#id2)したので、デフォルトでPython3.8が入っている18.04から20.4に変更します。
