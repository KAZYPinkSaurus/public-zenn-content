...
aws-neuron-toolsについてくるスクリプト使って、Inferentiaチップに関するメトリクスをCloud Watchに送信すると、デフォルトでは約25個のカスタムメトリクスができます。
...
```bash
neuron-monitor -c monitor.conf | neuron-monitor-cloudwatch.py  --region ap-northeast-1
```
